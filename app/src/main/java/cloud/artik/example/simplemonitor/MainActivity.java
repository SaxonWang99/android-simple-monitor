    ...
    private final BroadcastReceiver mWSUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ArtikCloudSession.WEBSOCKET_LIVE_ONOPEN.equals(action)) {
                ...
                Notification notification = intent.getParcelableExtra(ArtikCloudSession.NOTIFICATION);

                try {
                    JSONObject jsonObject = new JSONObject(status);
                    String isOnFire = jsonObject.getString("onFire");
                    if(!"false".equalsIgnoreCase(isOnFire))
